...
def build_dataset(src_path, tgt_path):
    ...
    train_iter = BucketIterator(train, batch_size=20, sort_key=lambda x: len(x.tgt), shuffle=True)
    valid_iter = BucketIterator(valid, batch_size=20, sort_key=lambda x: len(x.tgt), shuffle=True)
